...
# Heart Disease Prediction Rate
    ...
    # Columns for input fields
        ...
        with col3:
            ...
            cp = st.selectbox('Chest Pain type/condition',
                          options2, format_func=lambda x: display2[x])
        ...
        with col3:
            ...
            fbs = st.selectbox('Fasting blood pressure > 120 Mg/dl',
                           options_yes_no, format_func=lambda x: display_yes_no[x])
        ...
        with col1:
            ...
            "Resting electrographic result", options4, format_func=lambda x: display4[x])
        ...
        with col3:
            ...
            exang = st.selectbox("Exercise induced angina", options_yes_no,
                             format_func=lambda x: display_yes_no[x], key=2)
        ...
        with col2:
            ...
            slope = st.selectbox("Slope of the peak exercise ST segment",
                             options6, format_func=lambda x: display6[x])
        ...
        with col3:
            ...
            ca = st.selectbox("Number of major vessels (0-3) colored by flourosopy",
                          options7, format_func=lambda x: display7[x])
        ...
        with col1:
            ...
            thal = st.selectbox(
            "Thal", options8, format_func=lambda x: display8[x])
    ...
    # creating a button for prediction
        ...
        if st.button('Heart Test Result'):
            inputFeatures = [age, sex, cp, trestbps, chol, fbs,
...
# *****************************************LUNG CANCER SECTION********************************************
    ...
    # Columns for input fields
        ...
        with col1:
            gender = st.selectbox("Gender", options_gen,
                              format_func=lambda x: display_gen[x], key=2)

        with col2:
            smoking = st.selectbox("Do the person smoke?", options_yes_no,
                               format_func=lambda x: display_yes_no[x], key=3)

        with col3:
            yellow_fingers = st.selectbox("Does the person have yellow fingers?",
                                      options_yes_no, format_func=lambda x: display_yes_no[x], key=4)

        with col4:
            anxiety = st.selectbox("Does the person have Anxiety issues?",
                               options_yes_no, format_func=lambda x: display_yes_no[x], key=5)

        with col1:
            peer_pressure = st.selectbox("Does the person have Peer Pressure problems?",
                                     options_yes_no, format_func=lambda x: display_yes_no[x], key=6)

        with col2:
            chr_disease = st.selectbox("Does the person have any chronic disease?",
                                   options_yes_no, format_func=lambda x: display_yes_no[x], key=7)

        with col3:
            fatigue = st.selectbox("Does the person get fatigue easily?",
                               options_yes_no, format_func=lambda x: display_yes_no[x], key=8)

        with col4:
            allergy = st.selectbox("Does the person have any allergies?",
                               options_yes_no, format_func=lambda x: display_yes_no[x], key=9)

        with col1:
            wheezing = st.selectbox("Does the person have wheezing problem?",
                                options_yes_no, format_func=lambda x: display_yes_no[x], key=10)

        with col2:
            alcohol_consumption = st.selectbox(
            "Does the person consume alcohol?", options_yes_no, format_func=lambda x: display_yes_no[x], key=11)

        with col3:
            coughing = st.selectbox("Does the person have coughing?",
                                options_yes_no, format_func=lambda x: display_yes_no[x], key=12)

        with col4:
            shortness_of_breath = st.selectbox(
            "Does the person have shortness in breathing?", options_yes_no, format_func=lambda x: display_yes_no[x], key=13)

        with col1:
            swallowing_difficulty = st.selectbox(
            "Does the person have yellow fingers?", options_yes_no, format_func=lambda x: display_yes_no[x], key=14)
...
# *****************************************CHRONIC KIDNEY DISEASE SECTION********************************************
    ...
    # Columns for input fields
        ...
        with col6:
            red_blood_cells = st.selectbox("What is the condition of Red blood cells?",
                                       options_normal_abnormal, format_func=lambda x: display_normal_abnormal[x], key=1)

        with col1:
            pus_cell = st.selectbox("What is the condition of Pus Cells ?", options_normal_abnormal,
                                format_func=lambda x: display_normal_abnormal[x], key=2)

        with col2:
            pus_cell_clumps = st.selectbox("Are Pus Cell Clumps present in the body?",
                                       options_present_notpresent, format_func=lambda x: display_present_notpresent[x], key=3)

        with col3:
            bacteria = st.selectbox("Is bacterial infection present in the body?",
                                options_present_notpresent, format_func=lambda x: display_present_notpresent[x], key=4)

        with col4:
            blood_glucose_random = st.number_input(
                'Blood Glucose random value', step=1.0)
        ...
        with col5:
            ...
            'White Blood Cell Count', step=10)

        with col6:
            red_blood_cell_count = st.number_input('Red Blood Cell Count', step=10)

        with col1:
            hypertension = st.selectbox("Is the person having Hypertension problems?",
                                    options_yes_no, format_func=lambda x: display_yes_no[x], key=16)

        with col2:
            diabetes_mellitus = st.selectbox("Is the person having Diabetes Mellitus problem?",
                                         options_yes_no, format_func=lambda x: display_yes_no[x], key=17)

        with col3:
            coronary_artery_disease = st.selectbox(
            "Is the person having Coronary Artery Disease?", options_yes_no, format_func=lambda x: display_yes_no[x], key=18)

        with col4:
            appetite = st.selectbox("What is the apetite condition of the patient?",
                                options_good_bad, format_func=lambda x: display_good_bad[x])

        with col5:
            peda_edema = st.selectbox("Is the person having Pedal Edema?",
                                  options_yes_no, format_func=lambda x: display_yes_no[x], key=19)

        with col6:
            anemia = st.selectbox("Is the person having Anemia?", options_yes_no,
                              format_func=lambda x: display_yes_no[x], key=20)
    ...
    # creating a button for prediction
        ...
        if st.button('Chronic Kidney Disease Test Result'):
            inputFeatures = [age, blood_pressure, specific_gravity, albumin, sugar, red_blood_cells, pus_cell, pus_cell_clumps, bacteria, blood_glucose_random, blood_urea, serum_creatinine, sodium,
                         potassium, haemoglobin, packed_cell_volume, white_blood_cell_count, red_blood_cell_count, hypertension, diabetes_mellitus, coronary_artery_disease, appetite, peda_edema, anemia]
...
# *****************************************LIVER DISEASE SECTION********************************************
    ...
    # Columns for input fields
        ...
        with col3:
            Total_Bilirubin = st.number_input(
                'Total Bilirubin value', step=0.5)

        with col1:
            Direct_Bilirubin = st.number_input(
                'Direct/Conjugated Bilirubin value', step=0.5)

        with col2:
            Alkaline_Phosphotase = st.number_input(
                'Alkaline Phosphotase value', step=10)

        with col3:
            Alamine_Aminotransferase = st.number_input(
            'Alanine Aminotransferase value', step=5)

        with col1:
            Aspartate_Aminotransferase = st.number_input(
            "Aspartate Aminotransferase value", step=5)
        ...
        with col1:
            ...
            "Albumin and Globulin Ratio", step=0.1)
    ...
    # creating a button for prediction
        ...
        if st.button('Liver Disease Test Result'):
            inputFeatures = [Age, Gender, Total_Bilirubin, Direct_Bilirubin, Alkaline_Phosphotase,
